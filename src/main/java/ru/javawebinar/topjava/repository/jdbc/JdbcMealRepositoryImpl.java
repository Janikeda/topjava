...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return jdbcTemplate.query("SELECT id, datetime, description, calories FROM meals WHERE userId = ? AND datetime >= ? AND " +
                "datetime <= ? ORDER BY datetime DESC ", ROW_MAPPER, userId, Timestamp.valueOf(startDate), Timestamp.valueOf(endDate));
    }
}